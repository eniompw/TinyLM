...
tok_embed = nn.Embedding(len(vocab), 256)                                               # token embedding lookup layer (256 is embed_dim)
...
params = list(tok_embed.parameters()) + list(pos_embed.parameters()) + list(transformer.parameters()) + list(model.parameters())
...
for epoch in range(2001):
    ...
    with torch.autocast('cuda', dtype=torch.float16):                                   # float16 mixed precision for speed
        input_embed = tok_embed(batch_inputs) + pos_embed(torch.arange(context_size))   # add token and positional embeddings
        logits = model(transformer(input_embed)[:, -1, :])                              # forward pass through transformer & linear layer
    ...
    if epoch % 200 == 0:
        with torch.no_grad(), torch.autocast('cuda', dtype=torch.float16):              # disable tracking during evaluation
            ...
            eval_input_embed = tok_embed(inputs[eval_idx]) + pos_embed(torch.arange(context_size))  # embed dataset subset
            preds = model(transformer(eval_input_embed)[:, -1, :]).argmax(1)            # dataset subset forward & argmax
...
def generate(num_chars=200, context=list(encoded_text[:context_size])):                 # start with true initial context
    ...
    for _ in range(num_chars):
        input_embed = tok_embed(torch.tensor([context])) + pos_embed(torch.arange(context_size))    # embed current window
        probabilities = torch.softmax(model(transformer(input_embed)[:, -1, :]) / 0.7, 1)       # apply temp 0.7 to pick higher-confidence tokens
