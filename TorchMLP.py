...
embedding = nn.Embedding(len(idx_to_char), embed_dim)                                            # token embedding lookup layer
mlp       = nn.Sequential(
    nn.Linear(context_size * embed_dim, hidden_dim), nn.ReLU(),                                  # maps context to hidden_dim & applies ReLU
    nn.Linear(hidden_dim, len(idx_to_char))                                                       # maps hidden state to logits (vocab length)
)
