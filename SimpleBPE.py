...
vocab_size   = 4000                                                                               # BPE vocabulary size trained on TinyStories
...
# --- Data & Tokenization ---
input_ids, target_ids, tokenizer, token_ids, vocab_size = load_tinystories_bpe(num_stories=num_stories, context_size=context_size, vocab_size=vocab_size) # previous tokens to predict next
...
def generate(num_tokens=100, context_ids=list(token_ids[:context_size]), temp=temp):              # start with true initial context
    for _ in range(num_tokens):
        ...
        context_ids = context_ids[1:] + [next_token]                                              # slide window forward
    return tokenizer.decode(context_ids)                                                          # decode full window to string
